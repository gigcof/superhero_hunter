...
const heroes = {
    ...
    "etag": "091f28a4b0ac9610252e1e64e5e1f7f9bddb9a49",
    "data": {
        "offset": 0,
        "limit": 20,
        "total": 1564,
...
async function fetchAllHeroes() {
    try {
        ...
        const apiUrl = `http://gateway.marvel.com/v1/public/characters?ts=${ts}&apikey=${publicKey}&hash=${hash}&limit=${10}`;
...
async function findHeroesByName(name) {
    try {
        const ts = new Date().getTime().toString(); // unique timestamp
        const hash = CryptoJS.MD5(ts + privateKey + publicKey).toString();
        const apiUrl = `http://gateway.marvel.com/v1/public/characters?name=${name}&ts=${ts}&apikey=${publicKey}&hash=${hash}`;
        const res = await fetch(apiUrl)
        const data = await res.json();
        return data.data.results;
    } catch (err) {
...
function renderSelectedHero(hero) {
    const heroDetails = document.querySelector('.hero-details');
    heroDetails.textContent = hero;
}
...
const renderHeroes = (heroes) => {
    heroes.forEach((hero) => {
        ...
        heroCard.className = 'card mb-3'
        heroCard.style.width = '18rem';
        const heroImg = document.createElement('img');
        heroImg.className = 'card-img-top';
        heroImg.src = hero.thumbnail.path + '.' + hero.thumbnail.extension;
        heroImg.alt = hero.name;
        const heroCardBody = document.createElement('div');
        heroCardBody.className = 'card-body';
        const heroTitle = document.createElement('h5');
        heroTitle.className = 'card-title';
        heroTitle.innerText = hero.name;
        const heroDescription = document.createElement('p');
        heroDescription.className = 'card-text';
        heroDescription.innerText = hero.description.substring(0, 50) + '...';
        const knowMoreBtn = document.createElement('a');
        knowMoreBtn.className = 'btn btn-primary';
        knowMoreBtn.innerText = 'Know More';
        knowMoreBtn.onclick = () => {
            localStorage.setItem('selectedHero', JSON.stringify(hero));
            window.location.href = 'hero.html?id=' + hero.id;
        }
        heroCardBody.appendChild(heroTitle);
        heroCardBody.appendChild(heroDescription);
        heroCardBody.appendChild(knowMoreBtn);
        heroCard.appendChild(heroImg);
        heroCard.appendChild(heroCardBody);
        colDiv.appendChild(heroCard);
        heroRow.appendChild(colDiv);
    })
}

async function main() {
    // const heroes = await fetchAllHeroes();
    renderHeroes(heroes.data.results);
}

// event listeners
searchBtn.addEventListener('click', async () => {
    const searchInput = document.querySelector('#searchInput').value;
    await findHeroesByName(searchInput);
    renderHeroes(heroes);
});
